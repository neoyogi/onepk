...
class ARTRouteStateListener(EventListener):
    """
    This interface represents a listener for receiving ARTRouteStateEvent.
    The class that is interested in processing a ARTRouteStateEvent event
    ...
    def handle_event(self, event, clientData):
        """
        Invoked when the ARTRouteStateEvent is received from the network element
        
        @param event: An event object which indicates that an event occurred
        in a network element
        @type event: L{ARTRouteStateEvent<onep.routing.ARTRouteStateEvent>}
