...
class RIBRouteStateListener(EventListener):
    """
    This interface represents a listener for receiving RIBRouteStateEvent.
    The class that is interested in processing a RIBRouteStateEvent event
    ...
    def handle_event(self, event, clientData):
        """
        Invoked when the a RIBRouteStateEvent  is received from the 
        network element.
        
        @param event: An event object which indicates that an event occurred
        in a network element.
        @type event: L{RIBRouteStateEvent<src.routing.RIBRouteStateEvent>}
