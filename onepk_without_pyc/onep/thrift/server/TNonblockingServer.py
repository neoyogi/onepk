...
class Connection:
    ...
    def _read_len(self):
        ...
        if len(self.message) == 4:
            ...
            if self.len < 0:
                logging.error("negative frame size, it seems client doesn't use FramedTransport")
                self.close()
            elif self.len == 0:
    ...
    def read(self):
        ...
        if self.status == WAIT_LEN:
            ...
            if len(read) == 0:
                logging.error("can't read frame from socket (get %d of %d bytes)" % (len(self.message), self.len))
                self.close()
                return 
            self.message += read
