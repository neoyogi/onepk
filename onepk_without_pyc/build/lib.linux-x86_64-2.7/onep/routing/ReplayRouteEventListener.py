...
class ReplayRouteEventListener(EventListener):
    """
    This interface represents a listener for receiving ReplayRouteEvent.
    The class that is interested in processing a ReplayRouteEvent event
    ...
    def handle_event(self, event, clientData):
        """
        Invoked when the ReplayRouteEvent is received from the network element
        
        @param event: An event object which indicates that an event occurred
        in a network element
        @type event: L{ReplayRouteEvent<src.routing.ReplayRouteEvent>}
